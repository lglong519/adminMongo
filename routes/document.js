...
router.post('/document/:conn/:db/:coll/insert_doc', (req, res, next) => {
...
router.post('/document/:conn/:db/:coll/edit_doc', (req, res, next) => {
	let connection_list = req.app.locals.dbConnections;
	let ejson = require('mongodb-extended-json');
	...
	mongo_db.collection(req.params.coll).save(eJsonData, (err, doc, lastErrorObject) => {
		if (err) {
...
router.post('/document/:conn/:db/:coll/mass_delete', (req, res, next) => {
	let ejson = require('mongodb-extended-json');
	let connection_list = req.app.locals.dbConnections;
...
// Deletes a document
router.post('/document/:conn/:db/:coll/doc_delete', (req, res, next) => {
	let connection_list = req.app.locals.dbConnections;
	...
	common.get_id_type(mongo_db, req.params.coll, req.body.doc_id, (err, result) => {
		if (result.doc) {
			mongo_db.collection(req.params.coll).remove({ _id: result.doc_id_type }, true, (err, docs) => {
				if (err || docs.result.n === 0) {
